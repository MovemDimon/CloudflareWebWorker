...
👇 Please select your currency to continue:`;
  ...
  break;
        ...
        case 'CURRENCY':
          ...
          reply = `Please select the network for ${user.data.currency}:`;
          keyboard = [
        ...
        case 'WALLET':
          ...
          user.data.merchant = user.data.network === 'TON' ? this.env.TON_MERCHANT_WALLET : this.env.ETH_MERCHANT_WALLET;
      ...
      await fetch(`https://api.telegram.org/bot${this.env.TELEGRAM_BOT_TOKEN}/${method}`, {
        method: 'POST',
  ...
  async sendViaWebSocket(userId, data) {
    return new Promise((resolve, reject) => {
      const wsUrl = `${this.env.WORKER_URL.replace('https', 'wss')}/ws?userId=${encodeURIComponent(userId)}&api_key=${this.env.WS_API_KEY}`;
